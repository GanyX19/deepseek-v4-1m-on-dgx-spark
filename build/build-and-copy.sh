...
GPU_ARCH_LIST="12.1a"
NETWORK_ARG=""
# Public upstream repo that publishes prebuilt vLLM/FlashInfer wheels for DGX Spark (sm_121a).
# Override with your own fork/mirror if desired.
...
usage() {
    ...
    echo "  -t, --tag <tag>               : Image tag (default: 'vllm-gb10-dsv4', 'vllm-gb10-dsv4-tf5' with --tf5, 'vllm-gb10-dsv4-mxfp4' with --exp-mxfp4)"
...
if [ "$IMAGE_TAG_SET" = false ]; then
    if [ "$PRE_TRANSFORMERS" = true ]; then
        IMAGE_TAG="vllm-gb10-dsv4-tf5"
    elif [ "$EXP_MXFP4" = true ]; then
        IMAGE_TAG="vllm-gb10-dsv4-mxfp4"
    fi
fi
...
if [ "$NO_BUILD" = false ]; then
    if [ "$EXP_MXFP4" = true ]; then
        echo "Experimental MXFP4 build is not part of this public recipe." >&2
        exit 1
    else
        # ----------------------------------------------------------
