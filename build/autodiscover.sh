...
detect_interfaces() {
    ...
    # Parse ibdev2netdev "Up" lines: "<ibdev> port 1 ==> <netdev> (Up)"
    ...
    if [[ "$num_up" -eq 2 ]]; then
        ...
        # Set IB_IF to your RoCE interfaces (configurable for mesh; set MESH_IB_IFS).
        if [[ -z "$IB_IF" ]]; then
            IB_IF="${MESH_IB_IFS:-ib0,ib1,ib2,ib3}"
            echo "  Detected IB_IF: $IB_IF"
        fi

        # Set ETH_IF for cluster coordination (configurable for mesh; set MESH_ETH_IF).
        if [[ -z "$ETH_IF" ]]; then
            mesh_eth="${MESH_ETH_IF:-eth0}"
            if ip addr show "$mesh_eth" 2>/dev/null | grep -q "inet "; then
                ETH_IF="$mesh_eth"
                echo "  Detected ETH_IF: $ETH_IF"
            else
                echo "Error: Mesh mode requires a coordination interface with an IP. Set MESH_ETH_IF to an up interface."
                return 1
            fi
        fi
...
detect_copy_hosts() {
    ...
    # Mesh mode: scan the direct IB interface subnets (set MESH_SCAN_IFS for your NICs)
    echo "Auto-detecting COPY_HOSTS on direct IB interfaces (mesh mode)..."

    local temp_file
    temp_file=$(mktemp)

    for iface in ${MESH_SCAN_IFS:-ib0 ib1}; do
        local cidr
